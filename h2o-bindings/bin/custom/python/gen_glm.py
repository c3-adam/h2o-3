...
def class_extensions():
    def get_regression_influence_diagnostics(self):
        ...
        if self.actual_params["influence"]=="dfbetas":
            return h2o.get_frame(self._model_json["output"]["_regression_influence_diagnostics"])
        else:
            raise H2OValueError("getRegressionInfluenceDiagnostics(model) can only be called if influence='dfbetas'.")
